                ...
                <div class="uk-grid uk-grid-medium">
                    @if(count($posts))
                        ...
                        @php
                            ...
                            $pLang = ($product) ? $product->languages->first() : null;
                            $affiliateUrl = ($product && !empty($product->link) && $pLang) 
                                ? write_url($pLang->pivot->canonical) . '?redirect=true' 
                                : ($fallbackAffiliateUrl ?? '');
                        @endphp

                        <div class="uk-width-1-1 uk-width-small-1-1 uk-width-medium-1-3 uk-width-large-1-4 mb25 js-news-item" data-affiliate="{{ $affiliateUrl }}" data-href="{{ $href }}">
                            <div class="news-item wow fadeInUp" data-wow-delay="{{ ($key % 4) * 0.1 }}s">
                                <a href="{{ $href }}" class="image img-cover js-news-link" title="{{ $title }}">
                                    <img src="{{ $image }}" alt="{{ $title }}">
                                </a>
                                <div class="info">
                                    <h3 class="title">
                                        <a href="{{ $href }}" title="{{ $title }}" class="js-news-link">{{ $title }}</a>
                                    </h3>
