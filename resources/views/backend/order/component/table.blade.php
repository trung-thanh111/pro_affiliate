        ...
        @if(isset($orders) && is_object($orders))
            ...
            <tr >
                ...
                @php
                    ...
                    $ship = isset($order['shipping']) ? $order['shipping'] : 0;
                @endphp
                <td class="text-right order-discount">
                    {{ convert_price($order->promotion['discount'] + $voucher_discount , true) }}
                </td>
                {{-- <td class="text-right order-shipping">
                    {{ convert_price($order->shipping, true) }}
                </td> --}}
                <td class="text-right order-total">
                    {{ convert_price($order->cart['cartTotal'] - $order->promotion['discount'] - $voucher_discount + $ship, true) }}
                </td>
