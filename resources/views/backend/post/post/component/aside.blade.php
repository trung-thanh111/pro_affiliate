    ...
    <div class="ibox-content">
        <div class="row mb15">
            <div class="col-lg-12 mb10">
                <div class="form-row">
                    <input type="text" name="logo" value="{{ old('logo', $post->logo ?? '') }}"
                        placeholder="Ảnh logo của trường" class="form-control upload-image" autocomplete="off">
                </div>
            </div>
            <div class="col-lg-6 mb10">
                <div class="form-row">
                    <input type="text" name="rate" value="{{ old('rate', $post->rate ?? '') }}"
                        placeholder="Đánh giá 1 - 100" class="form-control" autocomplete="off">
                </div>
            </div>
            <div class="col-lg-6 mb10">
                <div class="form-row">
                    <input type="text" name="comments" value="{{ old('comments', $post->comments ?? '') }}"
                        placeholder="Số đánh giá" class="form-control" autocomplete="off">
                </div>
            </div>
            <div class="col-lg-12 mb10">
                <div class="form-row">
                    <textarea name="extra" class="form-control" placeholder="Phần nội dung cơ bản, mỗi dòng cách nhau 1 dấu ,"
                        autocomplete="off" style="height:100px;">{{ old('extra', $post->extra ?? '') }}</textarea>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="row mb15">
            <div class="col-lg-12 mb10">
                <div class="form-row">
                    ...
                    <select name="is_review" class="form-control setupSelect2">
                        <option value="0"
                            {{ old('is_review', isset($post->is_review) ? $post->is_review : 0) == 0 ? 'selected' : '' }}>
                            Không</option>
                        <option value="1"
                            {{ old('is_review', isset($post->is_review) ? $post->is_review : 0) == 1 ? 'selected' : '' }}>
                            Có</option>
                    </select>
                </div>
            </div>
            <div class="col-lg-12 mb10">
                <div class="form-row">
                    ...
                    <select name="post_type" class="form-control setupSelect2">
                        @foreach (__('messages.post_type') as $key => $val)
                            <option
                                {{ $key == old('post_type', isset($post->post_type) ? $post->post_type : 'normal') ? 'selected' : '' }}
                                value="{{ $key }}">{{ $val }}</option>
                        @endforeach
                    </select>
                </div>
                        ...
                        @if (isset($products))
                            @foreach ($products as $product)
                                <option value="{{ $product->id }}"
                                    {{ old('product_id', isset($post->product_id) ? $post->product_id : '') == $product->id ? 'selected' : '' }}>
        ...
        <div class="row mb15">
            <div class="col-lg-12">
                <div class="form-row">
                    ...
                    <input type="text" name="icon" value="{{ old('icon', $post->icon ?? '') }}"
                        class="form-control upload-image" placeholder="Đường dẫn ảnh icon" autocomplete="off">
                </div>
            </div>
        </div>
        <div class="row mb15">
            <div class="col-lg-12">
                <div class="form-row">
                    ...
                    <input type="text" name="banner" value="{{ old('banner', $post->banner ?? '') }}"
                        class="form-control upload-image" placeholder="Đường dẫn ảnh banner sidebar"
                        autocomplete="off">
                </div>
            </div>
        </div>
                    ...
                    <select multiple name="related_posts[]" class="form-control setupSelect2">
                        @if (isset($posts))
                            @foreach ($posts as $item)
                                @if (isset($post) && $item->id == $post->id)
                                    @continue
                                @endif
                                <option value="{{ $item->id }}"
                                    {{ in_array($item->id, old('related_posts', $relatedPosts)) ? 'selected' : '' }}>
            ...
            <div class="col-lg-12">
                <div class="form-row">
                    <span class="image img-cover image-target"><img
                            src="{{ old('image', $post->image ?? '') ? old('image', $post->image ?? '') : 'backend/img/not-found.jpg' }}"
                            alt=""></span>
                    ...
                    <div class="mb15">
                        <select name="publish" class="form-control setupSelect2" id="">
                            @foreach (__('messages.publish') as $key => $val)
                                <option
                                    {{ $key == old('publish', isset($post->publish) ? $post->publish : '2') ? 'selected' : '' }}
                                    value="{{ $key }}">{{ $val }}</option>
                            @endforeach
                        </select>
                    </div>
