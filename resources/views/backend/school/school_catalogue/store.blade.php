...
@php
    $url = ($config['method'] == 'create') ? route('school_catalogue.store') : route('school_catalogue.update', $schoolCatalogue->id);
@endphp
<form action="{{ $url }}" method="post" class="box">
    ...
    <div class="wrapper wrapper-content animated fadeInRight">
        <div class="row">
            <div class="col-lg-9">
                @php
                    $translation = (isset($schoolCatalogue)) ? $schoolCatalogue->languages->first()->pivot : null;
                @endphp
                <x-backend.content 
            ...
            <div class="col-lg-3">
                <x-ibox heading="Chọn Danh mục cha">
                    <x-backend.select2 
                        ...
                        name="parent_id"
                        :selectedValue="$schoolCatalogue->parent_id ?? 0"
                    />
                </x-ibox>
                <x-ibox heading="Ảnh đại diện">
                    <x-backend.image-preview 
                        name="image"
                        :value="$schoolCatalogue->image ?? ''"
                    />
                </x-ibox>
                <x-ibox heading="Cấu hình nâng cao">
                    <x-backend.select2 
                        ...
                        name="publish"
                        :selectedValue="$schoolCatalogue->publish ?? 0"
                        class="mb10"
                    />
                    <x-backend.select2 
                        :options="__('messages.follow')"
                        name="follow"
                        :selectedValue="$schoolCatalogue->follow ?? 0"
                    />
                </x-ibox>
            </div>
        </div>
