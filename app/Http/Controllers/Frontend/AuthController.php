...
class AuthController extends FrontendController
{
    ...
    public function registerAccount(AuthRegisterRequest $request){
        if($this->customerService->create($request)){
           return redirect()->route('customer.login')->with('success','Đăng kí tài khoản thành công');
        }
    ...
    public function logout(Request $request)
    {
        Auth::guard('customer')->logout();
         // Laravel session cho guard này
        $sessionKey = Auth::guard('customer')->getName(); // => 'login_session_customer'
