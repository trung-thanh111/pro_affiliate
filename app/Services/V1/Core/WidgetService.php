...
class WidgetService extends BaseService
{
    ...
    private function getCataloguesWithLanguages(string $tableName, array $modelIds, int $language): Collection
    {
        ...
        $results = DB::select("
            SELECT 
                ...
                cl.content,
                (SELECT COUNT(*) FROM product_catalogue_product WHERE product_catalogue_id = c.id) as products_count
            FROM {$tableName} c
    ...
    public function getPromotionProducts(int $language = 1, int $limit = 8)
    {
        ...
        if ($primaryPromotion) {
            // Lấy IDs sản phẩm thuộc khuyến mãi này
            $productIds = DB::table('promotion_product_variant')
                ->where('promotion_id', $primaryPromotion->id)
                ->where('model', 'Product')
                ->pluck('product_id')
                ->toArray();
